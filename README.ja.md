...
- **Composer**（左）: プロンプト／ネガティブ（サイズ変更可能なエディタ）、モデル選択
  ...
  重ねがけ、各 LoRA に重みスライダ。ベースモデルと**別のアーキテクチャ向けのものは、両方の
  アーキテクチャが事実のときだけ**候補から外す（推測のアーキテクチャでは何も隠さない。CLI が生成前に
  使う規則と同じ）。**トリガーワードを表示し、プロンプトへ自動挿入**）、**Init image**
  ...
  上書き）、バッチ**枚数**（1〜50）。**License** セクションはベースモデル・使用中 LoRA・ControlNet の
  ライセンスを常時表示し、特記制約（非商用・派生不可 等）のあるものを強調します。
  クレジット表記が必要なモデルでは、記載すべき**クレジット文字列**（コピー可能な欄）も
  表示します — image-forge が画像メタデータに記録するのと同じ文字列です。
  「Generate」で生成 — 生成中は **Cancel**（⌘.）に変わります。キューに残りがある場合は
  停止方法を尋ねます: **Stop Now** は生成中の1枚を破棄して即座に停止（エンジンを再起動する
  ため次回はモデル再ロード）、**Finish Current Image** は生成中の1枚を完走させて
  ギャラリーに残し、残りのキューだけを破棄します。単発生成ならそのまま停止します。
  ステータスバーはバッチの進行（**3/50 — …**）を表示し、進捗バーはバッチ全体を示します。
...
- **モデル管理**（ImageForgeGUI → Manage Models…、⌘⇧M。またはモデル未導入時にピッカーの代わりに
  ...
  **モデルのソースページ（Civitai モデルページ or Hugging Face リポジトリ）を開くコンパス
  ...
  `page_url` 由来）。重みファイルが失われたモデルには **missing files** バッジが付き、
  不在パスと対処法を表示する。確認・削除できるよう一覧には残るが、Composer の各ピッカー
  からは除外される（下記）。
- **ロードできないモデルは提示しない。** image-forge の `models_dir` を移したのに
  レジストリを張り替えていない場合（またはモデルを置いたボリュームが未マウントの場合）、
  該当モデルはモデル／LoRA／ControlNet／アップスケーラの各ピッカーから消え、Model
  セクションが理由を説明する — ファイルを移動して `image-forge models relocate --apply`
  を実行するか、ボリュームをマウントする。ロード時に失敗するモデルを提示していたことが、
  この不具合を分かりにくくしていた（image-forge ADR-0008）。
